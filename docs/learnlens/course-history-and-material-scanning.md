...
Synthetic source builders can use `tests/support/material_scanning.py`: fixtures must explicitly call `enable_synthetic_scanning(monkeypatch)`. `approve_sourced_fixture_task` records a hash-bound synthetic receipt. It does not enable a policy globally. Fixture consumers creating new sourced approvals or querying operational retrieval must opt in; historical-only tests need not do so. Production code does not import these helpers.
