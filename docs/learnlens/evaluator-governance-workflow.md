...
Each import is an append-only evidence artifact, separate from criterion decisions and formal assessment results. Assessors inspect suggestions and independently enter their own decisions; the panel does not populate decisions or calculate a formal result. Selected moderation attempts withhold suggestions before ORIGINAL is recorded, then retain the original-versus-second visibility rules until independent review is complete. Candidate import/review uses the same restriction. Checks run under the course lock, including sampling capture, so opening the panel before the moderation queue cannot bypass blinding. The moderation UI does not request suggestions. Historic exposure is not retrospectively certified as blind.
...
Integration follow-up: the locked visibility race and selected pre-original access were reproduced and corrected in `a875e15` and `6980068`. Four focused tests passed; these overlap the earlier workflow cases and are not an additional application-suite total.
