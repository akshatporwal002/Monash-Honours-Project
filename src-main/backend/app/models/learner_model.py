...
class LearnerModelAnnotation(Base):
    """Protected learner context attached to exactly one immutable target."""
    ...
    __table_args__ = (
        ...
        Index("ix_learner_model_annotations_target", "target_kind", "evidence_id", "estimate_id"),
...
class LearnerModelCorrectionReview(Base):
    """Append-only educator review outcome over a learner annotation."""
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "(review_version = 1 AND expected_latest_review_version = 0 AND prior_review_id IS NULL) OR "
            "(review_version > 1 AND expected_latest_review_version = review_version - 1 AND prior_review_id IS NOT NULL)",
    ...
    action: Mapped[CorrectionAction] = mapped_column(
        enum_column(CorrectionAction, "learner_model_correction_review_action_type"), nullable=False
    )
...
class LearnerModelCorrectionSnapshotLink(Base):
    """Append-only link from an accepted review to its later full snapshot."""
    ...
    __table_args__ = (
        ...
        Index("ix_learner_model_correction_snapshot_links_snapshot", "snapshot_id", "occurred_at"),
