...
def calculate_metrics(cases: list[EvaluationCaseResult]) -> dict[str, float | int]:
    ...
    def hit_at(limit: int) -> float:
        return sum(bool(set(item.retrieved_chunk_ids[:limit]) & item.relevant_chunk_ids) for item in cases) / len(cases)

    reciprocal_ranks = []
    no_result_correct = []
    for item in cases:
        rank = next((index + 1 for index, chunk_id in enumerate(item.retrieved_chunk_ids) if chunk_id in item.relevant_chunk_ids), None)
    ...
    return {
        ...
        "no_result_accuracy": sum(no_result_correct) / len(no_result_correct) if no_result_correct else 1.0,
        "cross_course_leakage_count": sum(item.leaked_cross_course for item in cases),
        "p50_latency_ms": median(latencies), "p95_latency_ms": latencies[max(0, int(len(latencies) * 0.95) - 1)],
    }
