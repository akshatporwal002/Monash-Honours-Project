...
SCRATCH = ROOT / ".tmp-task22" / os.environ.get("TASK22_BROWSER_RUN", "browser")
SCRATCH.mkdir(parents=True, exist_ok=True)
os.environ.update(
    DATABASE_URL=f"sqlite:///{(SCRATCH / 'browser.db').as_posix()}",
    APP_ENV="development",
    FRONTEND_ORIGIN="http://localhost:5272",
    RAG_UPLOAD_DIR=str(SCRATCH / "uploads"),
    LLM_API_KEY="",
    RESEARCH_ENABLED="false",
    WORKER_ADAPTER_FACTORY="app.worker:build_offline_worker_adapters",
    LEARNING_EVENT_PSEUDONYM_SECRET="task22-browser-test-pseudonym-secret-32-bytes",
)
...
def main():
    ...
    threading.Thread(
        target=lambda: asyncio.run(create_configured_worker().run_forever()), daemon=True
    ).start()
    uvicorn.run(create_app(), host="127.0.0.1", port=8172)
