...
export function TaskView({
  ...
  const valid = mode === 'unsupported' || choicesUnavailable ? false : mode === 'structured' ? validStructured(task.structured_task, answer) : hasEpisode ? Boolean(episode.supported.application?.answer?.trim() || episode.supported.prediction?.answer?.trim() || episode.supported.reasoning?.trim() || episode.supported.explanation?.trim() || episode.supported.reflection?.trim()) : mode === 'mcq'
    ...
    <article className={cx('ll-root', styles.page)} aria-labelledby="task-title">
      ...
      <div className={styles.layout}>
        ...
        <section id="task-response" tabIndex={-1} className={styles.interaction} aria-label="Activity">
          ...
          {hasEpisode && <EpisodeFields standaloneTransfer={task.task_type === 'transfer' && !task.episode_plan} value={episode} state={episodeState} attempts={attempts ?? []} disabled={busy || draftLoading || workConflict || Boolean(task.assessment && !workStartId)} onChange={value => { setEpisode(value); touch() }} onCheckpoint={() => void recordPrediction()} onTransfer={() => void enterTransfer()} onTransferCheckpoint={() => void recordPrediction(true)} onTransferSimulation={() => void runTransferSimulation()} support={episodeState && <EpisodeSupport onRequest={presentation.support_amount === 'on_request'} format={presentation.format} detail={presentation.explanation_detail} taskId={task.id} workId={workStartId} state={episodeState} disabled={busy || draftLoading || workConflict || Boolean(task.assessment && !workStartId)} />} />}
