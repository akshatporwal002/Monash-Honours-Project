    ...
    <article className={cx('ll-root', styles.page)} aria-labelledby="task-title">
      ...
      <div className={styles.layout}>
        ...
        <section id="task-response" tabIndex={-1} className={styles.interaction} aria-label="Activity">
          ...
          {hasEpisode && <EpisodeFields standaloneTransfer={task.task_type === 'transfer' && !task.episode_plan} value={episode} state={episodeState} attempts={attempts ?? []} disabled={busy || draftLoading || workConflict || Boolean(task.assessment && !workStartId)} onChange={value => { setEpisode(value); touch() }} onCheckpoint={() => void recordPrediction()} onTransfer={() => void enterTransfer()} onTransferCheckpoint={() => void recordPrediction(true)} onTransferSimulation={() => void runTransferSimulation()} support={episodeState && <EpisodeSupport onRequest={presentation.support_amount === 'on_request'} taskId={task.id} workId={workStartId} state={episodeState} disabled={busy || draftLoading || workConflict || Boolean(task.assessment && !workStartId)} />} />}
