...
export function PreferenceWorkspace({ effective, disabled, onBreak, onRepeat }: {
  ...
  const guidance = effective.pathway_support_level === 'concept_cue'
    ? ['Explain the concept in your own words before checking your response.']
    : values.explanation_detail === 'detailed'
  ...
  return <Card heading="Workspace preferences">
    ...
    {!effective.transfer && <>
      ...
      <section aria-label="Optional workspace guidance"><details open={effective.pathway_support_level !== 'independent'}><summary>Optional guidance{effective.pathway_support_level ? ` (${effective.pathway_support_level.replaceAll('_', ' ')})` : ''}</summary>{values.format === 'stepwise' ? <ol>{guidance.map(line => <li key={line}>{line}</li>)}</ol> : <p>{guidance.join(' ')}</p>}</details></section>
    </>}
