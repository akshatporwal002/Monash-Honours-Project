...
test('quantum episode connects checked feedback, revision, model, activity and human result', async ({ page, browser, request }, testInfo) => {
  ...
  const diagnostics = await browserDiagnostics(page, [fixture.student_email, fixture.student_password, fixture.educator_email, fixture.educator_password])
  const errors: string[] = []
  page.on('pageerror', error => errors.push(error.message))
  try {
    diagnostics.phase('sign in')
    await signIn(page, 'Student', fixture.student_email, fixture.student_password)
    diagnostics.phase('open episode')
    ...
    diagnostics.phase('reload saved draft')
    await page.reload()
    ...
    diagnostics.phase('reload confirmed result')
    await page.reload()
    ...
    diagnostics.phase('open chosen activity')
    ...
    expect(errors, `Browser diagnostics: ${JSON.stringify(diagnostics.events)}`).toEqual([])
