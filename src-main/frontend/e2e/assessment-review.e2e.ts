...
const reviewActions = [
  { label: 'Withhold result', state: 'PROVISIONAL', result: 'PASS' },
  { label: 'Return for review', state: 'PROVISIONAL', result: 'PASS' },
  { label: 'Confirm result', state: 'CONFIRMED', result: 'PASS' },
  { label: 'Override result', state: 'OVERRIDDEN', result: 'INCOMPLETE' },
] as const
...
for (const reviewAction of reviewActions) {
  test(`assessor records ${reviewAction.label} from frozen evidence by keyboard`, async ({ page, assessmentReview }, testInfo) => {
    const actionLabel = reviewAction.label
    ...
    await expect(
      page
        .getByText(
          /^Provisional \u2014 awaiting assessor review$/,
        )
        .first(),
    ).toBeVisible()
    ...
    expect(new URL(actionResponse.url()).pathname).toBe(
      `/api/v1/assessment/decisions/${assessmentReview.decision_id}/review`,
    )
    expect(await actionResponse.json()).toMatchObject({
      result_state: reviewAction.state,
